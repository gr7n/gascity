...
request that may have entered never falls through to direct cleanup. It cannot
claim durable acceptance, operation lookup, or this row in full. P2.11/P2.11A
must supply those mechanisms before the row is complete.
...
by authoritative reread before projection advance.
...
not create a second stop owner or provider call.
...
follows orphan resolution and precedes required store shutdown.
...
discoverable paths, but it does not satisfy this row. Only P1.1B may claim it
after every supported store proves bounded empty-memory discovery.
