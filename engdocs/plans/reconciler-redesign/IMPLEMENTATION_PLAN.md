...
MemStore/FileStore/BdStore/CachingStore and the existing DoltLite read path all
prove filter/order/limit/error equivalence. An error cannot become an empty-
success snapshot; no caller mutates a shared result. The post-canonicalization
test would fail if the earlier snapshot or pre-repair unassigned-route slice
were reused. Filtered allocation is capped by the query limit. An
uncomparable custom store cannot panic or silently bypass the read budget.
Repeated consumers report one store/phase read error without losing
...
lost response requires authoritative reread.
...
view; commit-unknown blocks and rereads (`RC-STATE-001`).

**Verification:** red regression at the current helper and caller, failed/
ambiguous/success tables, and an end-to-end same-pass effect-count assertion.

**Dependencies:** None beyond current session characterization.

**Likely files:** `cmd/gc/session_reconciler.go` and its focused tests.
...
branch also acquires and retains the same-path lock after the controller exits
and through store shutdown so a same-path concurrent start cannot enter the
handoff gap. Exact response equality, not substring matching, recognizes `ok`.
...
overclaim cross-path exclusion. Managed and direct cleanup retain the lock
through required store shutdown. No default tmux socket is touched. This slice proves the
negative no-fallback part of `RC-CLI-004` plus the local socket-isolation part
of `RC-SHUT-005`; it explicitly does **not** claim durable acceptance,
operation lookup, or full `RC-CLI-001..005` conformance.

**Verification:** deterministic socket barrier/re-exec tests plus an isolated
city provider/store spy. No timing/log-polling assertion counts.

**Dependencies:** P0.13 characterization may land beside it; the safe patch
does not wait for P2.11 and adds no request schema.

**Likely files:** current `gc stop` command/controller client adapter and focused
process tests.
...
helper or return the cleanup error; a terminal-state write failure never closes
an undiscoverable partial root. No `_ = SetMetadata` remains for convergence
progress markers. `EventTerminated` and terminal-success `EventIteration` fire
only after state, close, and applicable dedup marker proof; recorder failure
...
but this slice does not claim that an empty-memory startup can discover it.
...
pending-next marker without repouring. A static test rejects ignored
`SetMetadata` progress writes. Record the startup query characterization that
closed rows are excluded; it is the red input to P1.1B, not evidence for
`RC-CRASH-002`.
...
**Likely files:** `internal/convergence/{create,handler,reconcile}.go` and
focused tests.
...
- [Conditional-write design](../beads-cas/DESIGN.md) and
  [requirements](../beads-cas/REQUIREMENTS.md) — existing revision/CAS contract
  to graduate rather than redesign.
