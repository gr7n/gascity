...
func TestBDVersionPins(t *testing.T) {
	...
	for name, moduleVersion := range map[string]string{
		"BD_IMAGE_X_CRYPTO_VERSION": bdImageCrypto,
		"BD_IMAGE_X_NET_VERSION":    bdImageNet,
	} {
		if !regexp.MustCompile(`^v\d+\.\d+\.\d+$`).MatchString(moduleVersion) {
			t.Fatalf("deps.env %s = %q, want a v-prefixed module version", name, moduleVersion)
		}
	}
	...
	agentImage := readFile(t, root, "contrib/k8s/Dockerfile.agent")
	if !strings.Contains(agentImage, ".github/scripts/build-bd-image.sh /out/bd") {
		t.Fatal("Dockerfile.agent must build the source-pinned bd binary inside BuildKit")
	}
