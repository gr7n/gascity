...
`), 0o644); err != nil {
	...
	var stdout, stderr bytes.Buffer
	code := doDoctorWithChecks(false, false, false, []string{"inherited-rig-split-brain"}, &stdout, &stderr)
...
func TestDoDoctorCheckFilterRunsOnlyNamedCheck(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	_ = doDoctorWithChecks(false, false, false, []string{"city-config"}, &stdout, &stderr)
	out := stdout.String() + stderr.String()
...
func TestMissingDoctorChecksReportsUnknownRequestedNames(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	code := doDoctorWithChecks(false, false, false, []string{"not-a-real-check"}, &stdout, &stderr)
	out := stdout.String() + stderr.String()
