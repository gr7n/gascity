...
branch.`,
  ...
  gc doctor --json`,
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			if doDoctorWithChecks(fix, verbose, jsonOut, checks, stdout, stderr) != 0 {
				return errExit
			}
			return nil
		},
...
func (c *missingDoctorChecksCheck) WarmupEligible() bool { return false }

func doDoctor(fix, verbose, jsonOut bool, stdout, stderr io.Writer) int {
	return doDoctorWithChecks(fix, verbose, jsonOut, nil, stdout, stderr)
}

func doDoctorWithChecks(fix, verbose, jsonOut bool, checks []string, stdout, stderr io.Writer) int {
