  ...
  gc build-image ~/bright-lights --tag my-city:latest
	  ...
	  gc build-image ~/bright-lights --tag registry.io/my-city:latest --push`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			result, code := doBuildImageWithResult(args, tag, baseImage, rigPaths, workspaces, push, contextOnly, out, stderr)
			if code != 0 {
				return errExit
			}
			if jsonOut {
				return writeCLIJSONLineOrErr(stdout, stderr, "gc build-image", buildImageJSONResult{
					...
					WorkspacePathCount: len(workspaces),
				})
			}
			return nil
		},
...
func doBuildImage(args []string, tag, baseImage string, rigPaths, workspacePaths []string, push, contextOnly bool, stdout, stderr io.Writer) int { //nolint:unparam // preserved for existing tests; command path needs doBuildImageWithResult.
	_, code := doBuildImageWithResult(args, tag, baseImage, rigPaths, workspacePaths, push, contextOnly, stdout, stderr)
	return code
}

func doBuildImageWithResult(args []string, tag, baseImage string, rigPaths, workspacePaths []string, push, contextOnly bool, stdout, stderr io.Writer) (buildImageRunResult, int) {
