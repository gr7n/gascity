...
| SESSION-RUNTIME-007 | Submit outcome codes | Asynchronous session submit distinguishes an ambiguous transport receipt (`delivery_unconfirmed`) from an accepted turn that immediately hit a provider limit (`provider_unavailable`), preserving retry/limit evidence without resending the prompt. | `internal/api/handler_session_submit_test.go`; `internal/runtime/tmux/codex_delivery_outcome_test.go` |
