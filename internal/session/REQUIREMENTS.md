...
| SESSION-RUNTIME-007 | Concrete multi-session prompt capability | An agent-backed session whose concrete `agent_name` is not itself a configured agent resolves prompt capability through its current multi-session template only when a controller-managed alias or canonical identity exactly binds that concrete name. Missing or mismatched binding and single-session templates fail closed; direct provider sessions remain provider-backed. | `internal/session/provider_resolution_test.go`; `internal/api/handler_session_submit_test.go`; `cmd/gc/cmd_session_test.go` |
