...
| SESSION-RUNTIME-006 | Rendered prompt receipt | A newly prepared session start persists the rendered template projection's `prompt_version` and `prompt_sha` through the session metadata boundary and projects them onto session reads and worker-operation events. The projection includes configured injected/appended template fragments and excludes runtime delivery envelopes such as beacons, assigned-skills appendices, and initial messages. A later observed prompt-less start clears stale receipt values. Legacy sessions without an observed receipt remain honestly absent. | `internal/session/prompt_receipt_test.go`; `internal/session/lifecycle_transition_test.go`; `cmd/gc/template_resolve_prompt_test.go`; `internal/worker/operation_events_1a_test.go`; `internal/api/handler_sessions_test.go` |
| SESSION-RUNTIME-007 | Wrapper provider family learning | An alive session on a custom wrapper whose configured provider has no known transcript family may learn exactly one known family from runtime process-name observations. A builtin ancestor, an already-known configured family, an empty observation, or multiple distinct observed families produces no write. The reconciler persists the learned family once through the typed session front door and folds it into the current `Info` snapshot. | `internal/session/submit_test.go`; `cmd/gc/session_beads_test.go` |
