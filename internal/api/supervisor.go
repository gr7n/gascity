...
func NewSupervisorMux(resolver CityResolver, initializer cityInitializer, readOnly bool, version, buildID string, startedAt time.Time) *SupervisorMux {
	...
	// multipart streaming with a larger body cap than JSON mutations, while
	// workspace services own their own HTTP contracts. Go 1.22+ mux
	// specificity routes typed Huma operations to their exact scoped paths.
