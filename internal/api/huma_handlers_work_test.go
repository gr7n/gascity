...
func TestWorkReviewDurablyDeduplicatesAtomicHumanResponse(t *testing.T) {
	...
	input := workReviewInput(created.ID, "review-request-1", "Approve", "email:operator@example.com")
	...
	conflict := workReviewInput(created.ID, "review-request-2", "Deny", "email:operator@example.com")
...
func TestWorkReviewNeverRetriesAfterAmbiguousBackendFailure(t *testing.T) {
	...
	state.stores = map[string]beads.Store{}
	server := &Server{state: state}
	input := workReviewInput(created.ID, "review-request-3", "Approve", "email:operator@example.com")
