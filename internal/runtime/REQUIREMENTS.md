...
| RUNTIME-CONTRACT-006 | Codex delivery outcomes | Codex prompt submission sends exactly one Enter. A matching post-submit `task_started` plus `user_message` rollout receipt proves acceptance even when a sub-poll turn is never visible as pane-busy; a terminal provider error after that receipt surfaces as typed `provider_unavailable` evidence and does not clean up the live session. Missing or unattributable evidence remains `delivery_unconfirmed` and is never retried automatically. | `internal/runtime/tmux/codex_delivery_outcome_test.go`; `internal/runtime/tmux/nudge_submit_confirm_integration_test.go`; `internal/runtime/tmux/startup_test.go` |
