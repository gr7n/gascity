...
| --- | --- | --- | --- | --- | --- | --- |
| Audit baseline | all tracked test source | fixed_sleep: 435 calls / 153 files (historical regex census: 447 / 157) | ga-80po0c.2 | tracked test source totals remain visible as audit evidence; ga-80po0c.2 owns this point-in-time source census | P0.4a | 2026-10-01 |
...
| Small debt ratchet | `cmd/gc` untagged test source | environment: 4082 calls / 180 files | ga-80po0c.2.1 | untagged Small cmd/gc environment call/file totals cannot grow; reductions must lower this baseline; non-Medium lexical owners restore or eliminate every process-environment mutation | D5/D6/E6 | 2026-10-01 |
...
| Small debt ratchet | all untagged test source | fixed_sleep: 282 calls / 110 files | ga-80po0c.2.1 | untagged Small fixed-sleep call/file totals cannot grow; reductions must lower this baseline; non-Medium lexical owners replace elapsed wall time with lifecycle signals | W1-W5 | 2026-10-01 |
...
The fast local runner also gives the broad unit package sweep an explicit
package timeout and bounded package parallelism. Override those only when
diagnosing a specific local bottleneck:

```bash
LOCAL_TEST_GO_TIMEOUT=30m LOCAL_TEST_GO_P=4 make test-fast-parallel
```
